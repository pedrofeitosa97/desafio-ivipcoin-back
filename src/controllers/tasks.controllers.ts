...
const createTaskController = async (
  ...
  try {
    ...
    const validatedTask = await createTaskSchema.validate(task, {
      abortEarly: false,
    });
...
const listTasksController = async (
  req: Request,
  res: Response
): Promise<Response> => {
...
const listTaskByIdController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const id: string = req.params.id;
  const tasks = await listTasksService();
  const filteredTasks: any = tasks.filter(
    (task: any) => task.owner.user_id === id
  );
...
const updateTaskController = async (
  ...
  try {
    const validatedUpdatedTask = await updateTaskSchema.validate(data, {
      abortEarly: false,
    });
